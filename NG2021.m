...
Alkstar_rate_std_depth=NaN(MC,size(G2_b,2),size(max_sigma0bins,2));
Alkstar_rate_r2=NaN(MC,size(G2_b,2),size(max_sigma0bins,2));
Nbofpoints=NaN(MC,size(G2_b,2),size(max_sigma0bins,2));
Alkstar_range=NaN(MC,size(G2_b,2),size(max_sigma0bins,2));
...
for h=1:MC %FIRST FOR-LOOP: Monte Carlo for-loop 
   ...
   %"random" values
    ...
    for i=1:size(G2_b,2) %SECOND FOR-LOOP: for each biome, computes CaCO3 dissolution rates
        ...
        for j=1:size(sigma0bins,2) %THIRD FOR-LOOP
        %j=1:steps:size(bsigma0)-steps+1 %comment out if binning method=1
